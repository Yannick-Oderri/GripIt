...
echo "Build step 7"
rm -fr $WORKSPACE/docker_overlay_generating_dockers
mkdir -p $WORKSPACE/docker_overlay_generating_dockers

# monitor all subprocesses and enforce termination
python3 -u $WORKSPACE/ros_buildfarm/scripts/subprocess_reaper.py $$ --cid-file $WORKSPACE/docker_overlay_generating_dockers/docker.cid > $WORKSPACE/docker_overlay_generating_dockers/subprocess_reaper.log 2>&1 &
...
python3 -u $WORKSPACE/ros_buildfarm/scripts/devel/run_devel_job.py --prerelease-overlay --rosdistro-index-url https://raw.githubusercontent.com/ros/rosdistro/master/index.yaml kinetic default prerelease ubuntu xenial amd64 --distribution-repository-urls http://repositories.ros.org/ubuntu/testing --distribution-repository-key-files $WORKSPACE/keys/0.key --dockerfile-dir $WORKSPACE/docker_overlay_generating_dockers
echo "# END SECTION"

echo "# BEGIN SECTION: Build Dockerfile - generating devel tasks"
cd $WORKSPACE/docker_overlay_generating_dockers
python3 -u $WORKSPACE/ros_buildfarm/scripts/misc/docker_pull_baseimage.py
docker build --force-rm -t devel_task_generation.kinetic_prerelease_overlay .
echo "# END SECTION"

echo "# BEGIN SECTION: Run Dockerfile - generating devel tasks"
rm -fr $WORKSPACE/docker_overlay_build_and_install
rm -fr $WORKSPACE/docker_overlay_build_and_test
mkdir -p $WORKSPACE/docker_overlay_build_and_install
mkdir -p $WORKSPACE/docker_overlay_build_and_test
docker run --rm  --cidfile=$WORKSPACE/docker_overlay_generating_dockers/docker.cid -e=HOME=/home/buildfarm -e=TRAVIS=$TRAVIS -e=ROS_BUILDFARM_PULL_REQUEST_BRANCH=$ROS_BUILDFARM_PULL_REQUEST_BRANCH -v $WORKSPACE/ros_buildfarm:/tmp/ros_buildfarm:ro -v $WORKSPACE/catkin_workspace:/tmp/catkin_workspace:ro -v $WORKSPACE/catkin_workspace_overlay:/tmp/catkin_workspace_overlay:ro -v $WORKSPACE/docker_overlay_build_and_install:/tmp/docker_build_and_install -v $WORKSPACE/docker_overlay_build_and_test:/tmp/docker_build_and_test -v ~/.ccache:/home/buildfarm/.ccache devel_task_generation.kinetic_prerelease_overlay
cd -
echo "# END SECTION"
echo ""
...
echo "Build step 8"
# monitor all subprocesses and enforce termination
python3 -u $WORKSPACE/ros_buildfarm/scripts/subprocess_reaper.py $$ --cid-file $WORKSPACE/docker_overlay_build_and_test/docker.cid > $WORKSPACE/docker_overlay_build_and_test/subprocess_reaper.log 2>&1 &
# sleep to give python time to startup
sleep 1

echo "# BEGIN SECTION: Build Dockerfile - build and test"
# build and run build_and_test Dockerfile
cd $WORKSPACE/docker_overlay_build_and_test
python3 -u $WORKSPACE/ros_buildfarm/scripts/misc/docker_pull_baseimage.py
docker build --force-rm -t devel_build_and_test.kinetic_prerelease_overlay .
echo "# END SECTION"

echo "# BEGIN SECTION: Run Dockerfile - build and test"
docker run --rm  --cidfile=$WORKSPACE/docker_overlay_build_and_test/docker.cid -e=TRAVIS=$TRAVIS -v $WORKSPACE/ros_buildfarm:/tmp/ros_buildfarm:ro -v $WORKSPACE/catkin_workspace:/tmp/catkin_workspace:ro -v $WORKSPACE/catkin_workspace_overlay:/tmp/catkin_workspace_overlay devel_build_and_test.kinetic_prerelease_overlay
cd -
echo "# END SECTION"
echo ""
...
if [ -d "$WORKSPACE/catkin_workspace_overlay/test_results" ]; then
    ...
    catkin_test_results_CMD="catkin_test_results $WORKSPACE/catkin_workspace_overlay/test_results --all"
